...
class LocationsContainer:
    ...
    def find_nearest_five(self, current_id, max_demand):
        ...
        return [found, closest_5]

    def smaller_distance_remaining(self, start_id, distance):
        """
        :param distance:
        :param start_id:
        :return: a List of distances between the start_id node and all unvisited nodes
        """
        for i in range(len(self.locations)):
            if self.locations[i].times_visited == 0:
                if self.get_distance(start_id, i) < distance:
                    return True
